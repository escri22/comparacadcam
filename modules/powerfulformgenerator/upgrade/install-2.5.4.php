...
function upgrade_module_2_5_4($object)
{
    // Update database :)
    try {
        Db::getInstance()->execute(
            'ALTER TABLE `'._DB_PREFIX_.'pfg_lang`
            ADD COLUMN `send_label` varchar(255) NULL AFTER `success`,
            ADD COLUMN `unauth_redirect_url` varchar(255) NULL AFTER `message_admin`;'
        );
    } catch (Exception $e) {

...
class PosListCateProduct extends Module implements WidgetInterface
{
	...
	protected function installDemoData()
	{
		...
		for ($i = 1; $i <= 1; ++$i)
		{
			...
			$item->position = $i;
			$item->active = 1;
			$item->id_category = $i +1 ;
			$item->list_subcategories= '8';
			foreach ($languages as $language)
			{
				if($i==1){
					$item->description[$language['id_lang']]='<div class="static_bottom">
					<div class="row">
					<div class="box1 col col-lg-6 col-md-6 col-xs-12">
					<div class="banner-box"><a href="#"> <img src="/pos_boxstore/img/cms/img5_home3.jpg" alt="" class="img-responsive"></a></div>
					</div>
					<div class="col col-lg-6 col-md-6 col-xs-12 box2">
					<div class="banner-box"><a href="#"> <img src="/pos_boxstore/img/cms/img6_home3.jpg" alt="" class="img-responsive"></a></div>
					</div>
					</div>
					</div>';
				}elseif($i==2){
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
	   ...
	   $item['description'] = str_replace('/pos_boxstore/',__PS_BASE_URI__,$item['description']);
	  }
        if (is_array($items))
			foreach ($items as &$item)
			{	
	...
	{	
		...
		foreach ($languages as $lang)
		{
			...
			$fields['description'][$lang['id_lang']] = str_replace('/pos_boxstore/',__PS_BASE_URI__,$fields['description'][$lang['id_lang']]);
		}
